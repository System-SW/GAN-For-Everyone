...
class MapDataset(data.Dataset):
    def __init__(self,
        ...
        self.transform_mask = A.Compose(
            [
                A.Normalize(mean=[0.5 for _ in range(dataset.IMAGE_CHANNELS)],
    ...
    def __getitem__(self, index):
        ...
        augmentations = self.both_transform(
            ...
            image0=np.array(target_image))

        input_image = augmentations["image"]
        target_image = augmentations["image0"]

        input_image = self.transform_input(
            image=input_image)['image']
        target_image = self.transform_mask(
...
class AnimeDataset(data.Dataset):
    def __init__(self, dataset) -> None:
        ...
        for root, _, files in os.walk(os.path.join(dataset.DIR, 'line')):
            self.line_paths = sorted([os.path.join(root, filename)
                                      for filename in files])
        for root, _, files in os.walk(os.path.join(dataset.DIR, 'color')):
            self.color_paths = sorted([os.path.join(root, filename)
                                       for filename in files])
    ...
    def __getitem__(self, index):
        ...
        augmentations = self.both_transform(
            image=np.array(line_image),
            image0=np.array(color_image))
