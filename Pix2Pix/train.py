...
class Pix2Pix(Template):
    def __init__(self):
        ...
        self.restore_checkopint(
            cktp_path=hp.CKPT_PATH,
            gen=self.gen,
            disc=self.disc,
    ...
    def train(self):
        ...
        for inputs, targets in val_loader:
            FIX_DATA = (inputs.to(self.device)[:32], targets.to(self.device)[:32])
            break
            ...
            for batch_idx, (inputs, targets) in pbar:
                ...
                with torch.no_grad():
                    if batch_idx % hp.LOG_INTERVAL == 0:
                        ...
                        self.tb.add_scalar(
                            "TRAIN/L1_LOSS", l1_loss.item(), global_step=self.itr
                        )
                        self.tb.add_scalar(
                            "TRAIN/ADV_LOSS", adv_loss.item(), global_step=self.itr
                        )
    ...
    def test(self, inputs, targets):
        ...
        save_image = torch.cat([img_grid_fake, img_grid_real, img_grid_input], dim=1)

        self.tb.add_image("TEST/images", save_image, global_step=self.itr)
