...
class EBGAN(Template):
    def __init__(self):
        super().__init__(
            device=hp.DEVICE, seed=hp.SEED, model_name='EBGAN')
        ...
        self.add_image = lambda tag, image: self.tb.add_image(
            tag,
            torchvision.utils.make_grid(image[:hp.SAMPLE_SIZE],
                                        normalize=True),
    ...
    def pullaway_loss(self, embeddings):
        ...
        similarity = torch.matmul(
            normalized_embeddings, normalized_embeddings.transpose(1, 0)) ** 2
        batch_size = embeddings.size()[0]
        pt_loss = (torch.sum(similarity) - batch_size) / \
        ...
        for epoch in range(hp.NUM_EPOCHS):
            ...
            for batch_idx, (real, _) in pbar:
                ...
                with torch.cuda.amp.autocast():
                    ...
                    lossD = lossD_real + \
                        torch.clip(hp.MARGIN - lossD_fake, min=0)  # max(a,0)
                ...
                with torch.cuda.amp.autocast():
                    ...
                    lossG = self.MSE_loss(output_, fake.detach()) + \
                ...
                with torch.no_grad():
                    if batch_idx % hp.LOG_INTERVAL == 0:
                        self.tb.add_scalar('LossD', lossD.item(),
                                           global_step=self.itr)
                        self.tb.add_scalar('LossD/real', lossD_real.item(),
                                           global_step=self.itr)
                        self.tb.add_scalar('LossD/fake', lossD_fake.item(),
                                           global_step=self.itr)

                        self.tb.add_scalar('LossG', lossG.item(),
                                           global_step=self.itr)
